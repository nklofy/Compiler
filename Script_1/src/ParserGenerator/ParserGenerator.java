...
public class ParserGenerator {
	...
	private boolean getFollow(){//not used
		Symbol sym_e=gen_symbols.get("e");
	...
	private boolean getCCs(){
		...
		while(loop){
			...
			for(Item item:cc.items){	
				...
				cc.goto_tb.put(item.symbols.get(item.position), new_cc);
				getClosure(new_cc);
				if(!new_cc.in_table){
					gen_CCs.add(new_cc);
	...
	private void generateActionTable(){
		for(CC cc:gen_CCs){
			...
			for(Symbol token:gen_tokens){
				createActionTable(table,cc,token);
			}
			for(Symbol sym:gen_NTs){
				createGotoTable(table,cc,sym);
			}
